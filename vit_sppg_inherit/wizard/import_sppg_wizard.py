...
class ImportMitraWizard(models.TransientModel):
    _name = 'vit.import.mitra.wizard'
    _description = 'Wizard to Import Mitra SPPG'
            ...
            for row_number, row in enumerate(data_rows, start=2):
                try:
                    ...
                    if not row or len(row) < 8:
                        _logger.warning(f"Row {row_number}: Insufficient columns (got {len(row)}, need at least 8)")
                        skipped_count += 1
                        continue
                    
                    # Check required fields
                    # import pdb;pdb.set_trace()
                    id_mitra = str(row[0]).strip() if len(row) > 0 and row[0] else ''
                    name = str(row[2]).strip() if len(row) > 2 and row[2] else ''
                    provinsi_name = str(row[3]).strip() if len(row) > 3 and row[3] else ''
                    kab_kota_name = str(row[4]).strip() if len(row) > 4 and row[4] else ''
                    
                    # Skip if essential fields are empty
                    if not id_mitra or not name or not provinsi_name or not kab_kota_name:
                        _logger.warning(f"Row {row_number}: Missing required fields - ID: '{id_mitra}', Name: '{name}', Provinsi: '{provinsi_name}', Kab/Kota: '{kab_kota_name}'")
                        skipped_count += 1
                        continue
                    ...
                    try:
                        mitra_vals = {
                            ...
                            'jenis': str(row[8]).strip() if len(row) > 8 and row[8] else '',
                            'alamat': str(row[7]).strip() if len(row) > 7 and row[7] else '',
                            'status': str(row[9]).strip() if len(row) > 9 and row[9] else '',
                            'mulai_opsnal': str(row[10]).strip() if len(row) > 10 and row[10] else '',
                            'ka_sppg': str(row[11]).strip() if len(row) > 11 and row[11] else '',
                            'hp_sppg': str(row[12]).strip() if len(row) > 12 and row[12] else '',
                            'mail_sppg': str(row[13]).strip() if len(row) > 13 and row[13] else '',
                            'yayasan': str(row[14]).strip() if len(row) > 14 and row[14] else '',
                            'provinsi_id': provinsi.id,
                            'kab_kota_id': kab_kota.id,
                        }

                        existing_mitra = Mitra.search([('id_mitra', '=', id_mitra)], limit=1)
                        if existing_mitra:
                            existing_mitra.write(mitra_vals)
                            updated_count += 1
                            _logger.info(f"Updated mitra: {id_mitra} - {name}")
                        else:
                            Mitra.create(mitra_vals)
                            created_count += 1
                            _logger.info(f"Created mitra: {id_mitra} - {name}")
                        ...
                        _logger.error(f"Row {row_number}: Failed to save mitra '{id_mitra}': {e}")
                        error_count += 1
                        continue
